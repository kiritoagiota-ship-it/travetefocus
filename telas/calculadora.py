...
class TelaCalculadora(Screen):
    ...
    def add_texto(self, t):
        ...
        (Animation(color=(0, 0.9, 1, 1), duration=0.05, transition='out_quad') +
         Animation(color=(1, 1, 1, 1),   duration=0.18, transition='out_quad')).start(lbl)

    def backspace(self):
        lbl = self.ids.calc_display
        t   = lbl.text
        if not t or t == "ERRO":
            lbl.text = ""; lbl.color = (1, 1, 1, 1); return
        lbl.text = t[:-1]
        Animation.cancel_all(lbl, 'color')
    ...
    def limpar(self):
        ...
        (Animation(color=(1, 0.1, 0.2, 1), duration=0.06, transition='out_quad') +
         Animation(color=(1, 1, 1, 1),     duration=0.25, transition='out_quad')).start(lbl)
        Clock.schedule_once(lambda _: setattr(lbl, 'text', ''), 0.06)

    def calcular(self):
        raw = self.ids.calc_display.text.replace('X', '*').replace('÷', '/')
        lbl = self.ids.calc_display
        if not raw or not all(c in self._SAFE for c in raw):
            lbl.text = "ERRO"; self._flash_erro(lbl); return
        try:
            ...
            lbl.text = "ERRO"; self._flash_erro(lbl)
